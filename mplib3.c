...
int inicializarMPlib(int capacidade_de_cada_canal, int ntasks) {
  ...
  number_of_channels = ntasks*ntasks;
  channel_array    = (Channel_t**) malloc (sizeof(Channel_t*)*number_of_channels);
  mutex = (pthread_mutex_t*) malloc (sizeof(pthread_mutex_t)*number_of_channels);
  wait_for_messages = (pthread_cond_t*) malloc (sizeof(pthread_cond_t)*number_of_channels);
  wait_for_free_space = (pthread_cond_t*) malloc (sizeof(pthread_cond_t)*number_of_channels);
...
int receberMensagem(int tarefaOrig, int tarefaDest, void *buffer, int tamanho) {
  Channel_t      *channel;
  Message_t      *mess;
  int            copysize, mutex_id = tarefaDest*number_of_tasks+tarefaOrig;
  ...
  while (!mess) {
    if(pthread_cond_wait(&wait_for_messages[mutex_id], &mutex[mutex_id]) != 0) {
  ...
  if(pthread_cond_broadcast(&wait_for_free_space[mutex_id]) != 0) {
...
int enviarMensagem(int tarefaOrig, int tarefaDest, void *msg, int tamanho) {
  Channel_t      *channel;
  Message_t      *mess;
  int 			 mutex_id = tarefaDest*number_of_tasks+tarefaOrig;
  ...
  if (channel_capacity >0) {
    while (leQueSize(channel->message_list) >= channel_capacity) {
      if(pthread_cond_wait(&wait_for_free_space[mutex_id], &mutex[mutex_id]) != 0) {
        fprintf(stderr, "\nErro ao esperar pela variável de condição\n");
        return -1;
      }
    }
  ...
  pthread_cond_broadcast(&wait_for_messages[mutex_id]);
